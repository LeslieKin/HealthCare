...
public class RegisterPage extends Page implements TextWatcher, IBaseAView {
    ...
    protected void onPageInit() {
        bindClickEvents(
                ...
                R.id.textview_register_agreement,
    ...
    protected void handleClickEvents(View v) {
        switch (v.getId()) {
            ...
            case R.id.textview_register_agreement:
                mActivity.startActivity(new Intent(CommonActions.READ_AGREEMENT));
                break;

            case R.id.button_next_step:
                if (isChecked(R.id.checkbox_register_agreement)) {
                    showConfirmPhoneDialogFragment(getText(R.id.edittext_phone));
                } else {
                    showToast("你尚未同意微康用户注册协议");
                }
                break;
        }
    }

    private void showConfirmPhoneDialogFragment(String phone) {
        ...
        fragment.setOnDialongConfirmListener(new ConfirmPhoneDialogFragment.OnDialogConfirmListener() {
            ...
            public void onDialogConfirm() {
                mActivity.startActivity(new Intent(CommonActions.SEND_VERICODE).putExtra(CommonKeys.PHONE, phoneNumber)
                        .putExtra(CommonKeys.TYPE, SendVericodeType.REGISTER));
                fragment.dismiss();
            }
